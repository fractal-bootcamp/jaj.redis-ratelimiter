...
const TokenBucket: React.FC = () => {
    const [rateLimited, setRateLimited] = useState<boolean>(false);
    const [lastUpdated, setLastUpdated] = useState<Date | null>(null);

    useEffect(() => {
        socket.on('rateLimitEvent', (data) => {
            if (data.algorithm === 'tokenBucket') {
    ...
    const handleMakeRequest = async () => {
        try {
            await fetch(`${BACKEND_URL}/token-bucket`, {
                method: 'POST',
            });
        } catch (error) {
    ...
    return (
        <div className="flex flex-col md:flex-row">
            <div className="md:w-1/3 pr-4 mb-4 md:mb-0">
                <AlgorithmInfo algorithm="tokenBucket" />
            </div>
            <div className="md:w-2/3">
                <h2 className="text-2xl font-semibold mb-4">Token Bucket Algorithm</h2>
                <p className="mb-2">
                    Status:
