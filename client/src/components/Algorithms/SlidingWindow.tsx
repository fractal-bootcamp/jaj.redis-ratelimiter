...
const SlidingWindow: React.FC = () => {
    ...
    useEffect(() => {
        socket.on('rateLimitEvent', (data) => {
            if (data.algorithm === 'slidingWindow') {
    ...
    const handleMakeRequest = async () => {
        try {
            await fetch(`${BACKEND_URL}/sliding-window`, {
                method: 'POST',
            });
        } catch (error) {
    ...
    return (
        <div className="flex flex-col md:flex-row">
            <div className="md:w-1/3 pr-4 mb-4 md:mb-0">
                <AlgorithmInfo algorithm="slidingWindow" />
            </div>
            <div className="md:w-2/3">
                <h2 className="text-2xl font-semibold mb-4">Sliding Window Algorithm</h2>
                <p className="mb-2">
                    Status:
